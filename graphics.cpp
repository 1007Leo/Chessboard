...
void Graphics::handle_events(Board* board)
{
	...
	case SDL_MOUSEBUTTONDOWN:
		if (!f_lmb_down && event.button.button == SDL_BUTTON_LEFT && !board->game_over())
		{
			...
			for (auto& piece : f_pieces)
			{
				if (SDL_PointInRect(&f_mouse_pos, piece.rectangle) && piece.render_state == rendered && piece.color == board->get_current_turn())
				//if (SDL_PointInRect(&f_mouse_pos, piece.rectangle) && piece.render_state == rendered) // w/o move order
				{
